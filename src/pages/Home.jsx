...
export default function Home() {
  ...
  return (
    // 📍 FIX: ขยาย max-w เป็น 1600px และเพิ่ม gap-12 ถ่างซ้ายขวาให้สุด
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-12 max-w-[1600px] 2xl:max-w-[1800px] mx-auto pb-20 px-4 sm:px-6">
      ...
      {/* 📍 MIDDLE COLUMN: Feed Core (กว้างขึ้น ไม่โดนบีบแล้ว) */}
      <div className="flex-1 min-w-0 w-full max-w-4xl mx-auto space-y-6 md:space-y-8">
        
        {/* 📍 MOBILE ONLY: Navigation & Filters (กู้คืนของที่หายไป) */}
        <div className="lg:hidden space-y-3 mb-6">
          ...
          <div className="flex items-center space-x-2 overflow-x-auto hide-scrollbar pb-1">
            ...
            {/* 📍 FIX: กู้คืนปุ่ม Add News บนมือถือ */}
            {state.view === 'news' && (
              <button onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-add-news' }))} className="btn-press px-4 py-2 rounded-lg text-white text-[10px] font-bold shadow-md flex items-center gap-1.5 whitespace-nowrap ml-auto" style={{ background: 'var(--primary-glow)' }}>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex justify-between items-center pt-3 border-t border-[var(--border-line)] mt-3">
                  ...
                  <button onClick={handleQuickPost} className="btn-press px-6 py-2 sm:px-8 sm:py-2.5 rounded-xl text-white font-bold text-xs sm:text-sm shadow-md relative z-10" style={{ background: 'var(--primary-glow)' }}>Post</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6" : "flex flex-col space-y-4 sm:space-y-6"}>
            {filteredData.map(item => {
              ...
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-5 sm:p-8 cursor-pointer flex flex-col justify-between h-[240px] sm:h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-4 sm:mb-5">
                        <span className="text-[9px] sm:text-[10px] font-bold uppercase text-sub surface-bg px-3 py-1.5 rounded-full border border-[var(--border-line)] shadow-sm pointer-events-none">{item.loc}</span>
                        <button onClick={(e) => handleReport(e, item)} className="relative z-10 text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-1.5 hover:bg-red-500/10 rounded-lg shrink-0" title="Report this post">
                          <i data-lucide="flag" className="w-3.5 h-3.5 pointer-events-none"></i>
                        </button>
                      </div>
                      <h3 className="text-lg sm:text-2xl font-black text-prime mb-1.5 sm:mb-2 line-clamp-1 break-words pointer-events-none">{item.title}</h3>
                    ...
                    <div className="flex justify-between items-end mt-auto pt-4 sm:pt-5 border-t border-[var(--border-line)]/50">
                      <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 hover:opacity-80 transition cursor-pointer flex items-center space-x-2 sm:space-x-3 min-w-0">
                         {renderAvatar(item.avatar || item.host[0], "w-8 h-8 rounded-full text-xs shadow-sm shrink-0", item.host[0])}
                         <div className="min-w-0">
                           ...
                           <span className="text-xs sm:text-sm font-bold text-prime hover:underline truncate block pointer-events-none">{item.host}</span>
                         </div>
                      </div>
                      <button onClick={(e) => handleApply(e, item)} className="relative z-10 btn-press px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl surface-bg border border-[var(--border-line)] flex items-center justify-center hover:border-[var(--primary-glow)] hover:text-[var(--primary-glow)] text-prime shadow-sm transition-all duration-300 font-bold text-[10px] sm:text-xs gap-1.5 sm:gap-2 shrink-0">
              ...
              return (
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-5 sm:p-8 cursor-pointer relative group">
                  <div className="flex justify-between items-start mb-4 sm:mb-5">
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 flex items-center space-x-3 sm:space-x-4 hover:opacity-80 transition cursor-pointer min-w-0">
                      {renderAvatar(item.avatar || item.host[0], "w-10 h-10 sm:w-12 sm:h-12 rounded-full text-sm sm:text-base shadow-sm shrink-0", item.host[0])}
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm sm:text-base font-bold text-prime hover:underline truncate">{item.host}</span>
                        <span className="text-[9px] sm:text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5">
                  ...
                  <h3 className="text-lg sm:text-2xl font-black text-prime mb-2 sm:mb-3 leading-tight break-words pointer-events-none">{item.title}</h3>
                  <p className="text-xs sm:text-base text-sub mb-4 sm:mb-6 leading-relaxed font-medium break-words whitespace-pre-wrap pointer-events-none">{item.desc}</p>
                  
                  {item.image && (
                    <div className="mb-4 sm:mb-6 rounded-2xl overflow-hidden border border-[var(--border-line)] max-h-64 sm:max-h-80 pointer-events-none">
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-3 sm:pt-5">
                    {state.view === 'community' && (
                      <div className="flex space-x-4 sm:space-x-6 text-sub text-[10px] sm:text-xs font-bold">
                        <button onClick={(e) => handleLike(e, item.id)} className={`relative z-10 btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-white'}`}>
                          <i data-lucide="heart" className={`w-3.5 h-3.5 sm:w-5 sm:h-5 mr-1.5 pointer-events-none transition-all ${likedPosts[item.id] ? 'fill-red-500 animate-heart-burst' : ''}`}></i> 
                        ...
                        <span className="flex items-center hover:text-white transition pointer-events-none">
                          <i data-lucide="message-circle" className="w-3.5 h-3.5 sm:w-5 sm:h-5 mr-1.5"></i> {item.comments || 0}
                        </span>
                      </div>
                    )}
                    
                    {state.view === 'traders' && (
                      <span className={`flex items-center px-3 sm:px-4 py-1.5 rounded-full surface-bg border border-[var(--border-line)] shadow-sm text-[9px] sm:text-xs pointer-events-none ${item.sentiment === 'bullish' ? 'text-green-500' : 'text-red-500'}`}>
                        <i data-lucide={item.sentiment === 'bullish' ? 'trending-up' : 'trending-down'} className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5"></i> {item.sentiment?.toUpperCase()}
                      </span>
                    )}
                    
                    {state.view === 'news' && (
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <span className="font-mono text-[8px] sm:text-[10px] surface-bg border border-[var(--border-line)] px-2 py-1 rounded-md shadow-sm pointer-events-none">SRC: {item.source || 'SYS'}</span>
                        <button className="relative z-10 text-[9px] sm:text-xs font-bold text-[var(--primary-glow)] hover:underline flex items-center">
                          Read Article <i data-lucide="external-link" className="w-3 h-3 sm:w-3.5 sm:h-3.5 ml-1 sm:ml-1.5 pointer-events-none"></i>
                        </button>
                      </div>
                    )}
                    
                    <button onClick={(e) => handleShare(e)} className="relative z-10 btn-press text-sub hover:text-[var(--primary-glow)] p-1.5 sm:p-2 rounded-xl hover:bg-white/5 transition">
                      <i data-lucide="-2" className="w-3.5 h-3.5 sm:w-5 sm:h-5 pointer-events-none"></i>
                    </button>
                  </div>
                </div>
              );
      ...
      {/* 📍 RIGHT COLUMN: Widgets (Desktop Only - กู้คืนมา) */}
      ...
      {/* 📍 MOBILE ONLY: กู้คืน Widget ที่หายไป เอามาไว้ด้านล่างสุดของ Feed */}
      <div className="lg:hidden w-full space-y-6 pt-6 border-t border-[var(--border-line)]">
        <div className="bento-card rounded-3xl p-5">
          <h3 className="text-sm font-bold text-prime mb-4 flex items-center border-b border-[var(--border-line)] pb-2">
            <i data-lucide="trending-up" className="w-4 h-4 mr-2 text-[var(--primary-glow)]"></i> Trending Now
          </h3>
          <div className="flex overflow-x-auto hide-scrollbar gap-4 pb-2">
            <div className="shrink-0 surface-bg border border-[var(--border-line)] p-3 rounded-xl min-w-[140px]">
              <p className="text-xs font-bold text-prime">#NFP Week</p>
              <p className="text-[10px] text-sub mt-1">1,245 posts</p>
            </div>
            <div className="shrink-0 surface-bg border border-[var(--border-line)] p-3 rounded-xl min-w-[140px]">
              <p className="text-xs font-bold text-prime">Bitcoin Halving</p>
              <p className="text-[10px] text-sub mt-1">858 posts</p>
            </div>
            <div className="shrink-0 surface-bg border border-[var(--border-line)] p-3 rounded-xl min-w-[140px]">
              <p className="text-xs font-bold text-prime">Risk Management Tips</p>
              <p className="text-[10px] text-sub mt-1">432 posts</p>
            </div>
          </div>
        </div>
      </div>
