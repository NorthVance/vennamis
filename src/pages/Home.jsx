...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (activeFilter !== 'all') {
      if (activeFilter === 'remote') {
        result = result.filter(i => i.loc?.toLowerCase() === 'remote');
      } else if (activeFilter === 'high_budget') {
        result = result.filter(i => i.price >= 1000);
      } else if (activeFilter === 'top_rated') {
        result = result.filter(i => (i.likes || 0) > 100);
      } else if (activeFilter === 'bullish') {
        result = result.filter(i => i.sentiment === 'bullish');
      }
    }
  ...
  const handleApply = (e, item) => { 
    ...
    e.stopPropagation(); 
    if (!state.user) {
      return setState(prev => ({ ...prev, activeModal: 'modal-login' }));
    }
  ...
  const handleImageSelect = (e) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        return setState(prev => ({ ...prev, toast: { type: 'error', message: 'Image > 5MB' } }));
      }
  ...
  const renderAvatar = (avatarData, sizeClasses, fallbackText = 'U') => {
    if (avatarData && (avatarData.startsWith('http') || avatarData.startsWith('blob:'))) {
      return (
        <img 
          src={avatarData} 
          alt="Avatar" 
          className={`object-cover pointer-events-none ${sizeClasses}`} 
          onError={(e) => { 
    ...
    return (
      <div className={`flex items-center justify-center text-white font-bold bg-gray-800 pointer-events-none ${sizeClasses}`}>
        {avatarData ? avatarData[0] : fallbackText}
      </div>
    );
  };
      ...
      <section>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 mt-6 gap-4">
          ...
          <div className="flex items-center space-x-2 overflow-x-auto hide-scrollbar w-full sm:w-auto pb-1 sm:pb-0">
            <button onClick={() => setActiveFilter('all')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap ${activeFilter === 'all' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>
              All
            </button>
            
            {state.view === 'gigs' && (
              <>
                <button onClick={() => setActiveFilter('remote')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap ${activeFilter === 'remote' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>
                  Remote Only
                </button>
                <button onClick={() => setActiveFilter('high_budget')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'high_budget' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>
                  <i data-lucide="flame" className="w-3 h-3 mr-1.5 pointer-events-none"></i> High Budget
                </button>
              </>
            )}
            ...
            {state.view === 'community' && (
              <button onClick={() => setActiveFilter('top_rated')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'top_rated' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>
                <i data-lucide="trending-up" className="w-3 h-3 mr-1.5 pointer-events-none"></i> Top Rated
              </button>
            )}
            ...
            {state.view === 'traders' && (
              <button onClick={() => setActiveFilter('bullish')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'bullish' ? 'bg-green-500 text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>
                <i data-lucide="trending-up" className="w-3 h-3 mr-1.5 pointer-events-none"></i> Bullish Intel
              </button>
            )}
            ...
            {state.view === 'news' && (
              <button onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-add-news' }))} className="btn-press px-4 py-2 rounded-xl text-white text-[10px] sm:text-xs font-bold shadow-md flex items-center gap-1.5 hover-lift whitespace-nowrap ml-auto" style={{ background: 'var(--primary-glow)' }}>
                <i data-lucide="plus" className="w-3.5 h-3.5 pointer-events-none"></i> Add News
              </button>
            )}
          </div>
        </div>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <textarea 
                  ...
                  className="w-full bg-transparent text-sm text-prime outline-none resize-none placeholder-[var(--text-muted)] font-medium whitespace-pre-wrap"
                ></textarea>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-[var(--border-line)] mt-3">
                  <div className="flex space-x-2 text-sub">
                    ...
                    <button onClick={() => fileInputRef.current?.click()} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition" title="Attach Image">
                      <i data-lucide="image" className="w-4 h-4 pointer-events-none"></i>
                    </button>
                    <button onClick={() => setState(prev => ({...prev, toast: {type:'info', message:'Link attachment disabled.'}}))} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition">
                      <i data-lucide="link" className="w-4 h-4 pointer-events-none"></i>
                    </button>
                  </div>
                  <button onClick={handleQuickPost} className="btn-press px-8 py-2.5 rounded-xl text-white font-bold text-sm shadow-md relative z-10" style={{ background: 'var(--primary-glow)' }}>
                    Post
                  </button>
                </div>
              </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 border border-dashed border-[var(--border-line)] rounded-3xl">
            ...
            <button onClick={() => { setActiveFilter('all'); setSearchQuery(''); }} className="mt-4 px-4 py-2 rounded-lg surface-bg border border-[var(--border-line)] text-xs text-[var(--primary-glow)] font-bold hover-lift relative z-10">
              Clear Filters
            </button>
          </div>
        ) : (
                  ...
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-5">
                        <span className="text-[10px] font-bold uppercase text-sub surface-bg px-3 py-1.5 rounded-full border border-[var(--border-line)] shadow-sm pointer-events-none">
                          {item.loc}
                        </span>
                        <button onClick={(e) => handleReport(e, item)} className="relative z-10 text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report this post">
                    ...
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 flex items-center space-x-4 hover:opacity-80 transition cursor-pointer w-fit">
                      ...
                      <div className="flex flex-col pointer-events-none">
                        ...
                        <span className="text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5">
                          <i data-lucide="shield-check" className="w-3.5 h-3.5 mr-1 pointer-events-none"></i> Verified Content
                        </span>
                      </div>
                    </div>
                    <button onClick={(e) => handleReport(e, item)} className="relative z-10 text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report this post">
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 sm:pt-5">
                    {state.view === 'community' && (
                      <div className="flex space-x-6 text-sub text-xs font-bold">
                        <button onClick={(e) => handleLike(e, item.id)} className={`relative z-10 btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-[var(--primary-glow)]'}`}>
                        ...
                        <span className="flex items-center hover:text-[var(--primary-glow)] transition pointer-events-none">
