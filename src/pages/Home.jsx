...
import { 
    Search, MapPin, Sparkles, ArrowRight, ShieldCheck, Filter, 
    Heart, MessageCircle, TrendingUp, TrendingDown, ExternalLink, Flag, 
...
// ==========================================
// 🏢 MAIN COMPONENT: Home Multi-Verse Feed
// ==========================================
export default function Home() {
    ...
    // 🧠 COMPLEX MEMOIZATION: Multi-Verse Data Router
    const filteredData = useMemo(() => {
        // Route data based on current view
        let rawData = [];
        switch(state.view) {
    ...
    return (
        <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-12 sm:space-y-16">
            ...
            {/* ========================================== */}
            {/* 📋 DYNAMIC MULTI-VERSE FEED SECTION */}
            {/* ========================================== */}
            <section className={state.view === 'gigs' ? "max-w-6xl mx-auto" : "max-w-3xl mx-auto"}>
                ...
                {/* 📝 QUICK POST BLOCK (Only for Community & Traders) */}
                ...
                {isLoading ? (
                    ...
                    /* RENDER ENGINE: Switches between Grid (Gigs) and Feed (Social) */
                    <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "flex flex-col space-y-6"}>
                        {filteredData.map(item => (
                            <div key={item.id} 
                                    ...
                                    <>
                                        ...
                                        <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 mt-4">
                                            <div className="flex items-center space-x-2">
                                                <div className="w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-[10px] font-bold text-white">{item.host[0]}</div>
                                ...
                                {/* 2. SOCIAL FEED RENDER LOGIC (Community, Traders, News) */}
                                {state.view !== 'gigs' && (
                                    <>
                                        ...
                                        <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 mt-auto">
                                            {/* Contextual Footer based on View */}
