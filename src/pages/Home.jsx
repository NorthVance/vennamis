        ...
        {isLoading ? (
          ...
          <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6" : "flex flex-col space-y-4 sm:space-y-6"}>
            {filteredData.map(item => {
              ...
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[240px] group relative overflow-hidden">
                  ...
                  <div className="flex items-center space-x-6 text-sub text-xs font-medium border-t border-[var(--border-line)] pt-4">
                    {state.view === 'community' && (
                      <>
                        ...
                        <span className="flex items-center hover:text-white transition pointer-events-none"><i data-lucide="message-circle" className="w-4 h-4 mr-1.5"></i> {item.comments || 0}</span>
                      </>
                    )}
                    {state.view === 'traders' && (
