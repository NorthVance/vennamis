    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 max-w-[1400px] mx-auto pb-20 px-4">
      ...
      <div className="flex-1 min-w-0 max-w-4xl mx-auto w-full space-y-4 md:space-y-8">
        ...
        <div className="lg:hidden space-y-3 pt-2">
          {/* 📍 FIX: Mobile Nav Bar - Compact App Style (Flex-1, Flex-Col) */}
          <div className="glass-panel p-1.5 bg-black/5 border border-[var(--border-line)] rounded-xl flex gap-1 shadow-inner">
            {['gigs', 'community', 'traders', 'news'].map((nav) => (
              <button 
                key={nav} 
                onClick={() => setState(prev => ({ ...prev, view: nav }))} 
                className={`btn-press flex-1 flex flex-col items-center justify-center gap-1 py-2 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all duration-300 ${state.view === nav ? 'surface-bg text-prime shadow-sm border border-[var(--border-line)]' : 'text-sub hover:text-prime'}`}
              >
                {nav === 'gigs' && <i data-lucide="briefcase" className="w-4 h-4"></i>}
                {nav === 'community' && <i data-lucide="users" className="w-4 h-4"></i>}
                {nav === 'traders' && <i data-lucide="trending-up" className="w-4 h-4"></i>}
          ...
          <div className="flex items-center space-x-2 overflow-x-auto hide-scrollbar pb-1">
            ...
            {state.view === 'gigs' && (
              <>
                <button onClick={() => setActiveFilter('remote')} className={`btn-press px-4 py-2 rounded-lg text-[10px] font-bold transition whitespace-nowrap ${activeFilter === 'remote' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime'}`}>Remote</button>
        ...
        <div className="relative group">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[var(--bg-surface)] backdrop-blur-xl border border-[var(--border-line)] hover:border-[var(--primary-glow)]/50 rounded-xl sm:rounded-2xl pl-11 pr-4 py-3 sm:py-4 text-sm text-prime outline-none focus:border-[var(--primary-glow)] transition-all shadow-sm font-medium" placeholder="Search skills, posts, or news..." />
        </div>
