    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-12 xl:gap-16 w-full mx-auto pb-20">
      ...
      <div className="flex-1 min-w-0 space-y-6 md:space-y-8">
        ...
        <div className="lg:hidden flex flex-col items-center space-y-4 mb-2">
          <div className="glass-panel p-1 bg-[var(--bg-base)]/80 border border-[var(--border-line)] rounded-full flex items-center shadow-lg w-full max-w-sm mx-auto">
            {['gigs', 'community', 'traders', 'news'].map((nav) => (
          ...
          <div className="flex items-center justify-center space-x-2 overflow-x-auto hide-scrollbar pb-1 w-full">
        ...
        {/* UX: Apple Luxury Hero (Fix Overlap & Layout Shift) */}
        {state.view === 'gigs' && (
          <div className="bento-card rounded-[2.5rem] p-8 sm:p-12 lg:p-16 text-center relative overflow-hidden group min-h-[380px] flex flex-col justify-center items-center">
            ...
            <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full border surface-bg text-[10px] font-bold uppercase tracking-widest text-[var(--primary-glow)] mb-6 z-10 shadow-sm">
              <i data-lucide="shield-check" className="w-4 h-4"></i><span>{t.badge_secure}</span>
            </div>
            
            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tighter leading-tight text-prime w-full z-10">
              <span className="block mb-2">{t.hero_static}</span>
              {/* FIX: Prevent Jumping & Overlap (Lock Min-Height) */}
              <span className="block min-h-[2.5em] sm:min-h-[1.5em] w-full mt-2 lg:mt-4">
                <Typewriter />
              </span>
            </h1>
            
            <p className="text-sm sm:text-base lg:text-lg text-sub max-w-2xl mx-auto font-medium z-10 mt-4">{t.hero_sub}</p>
          </div>
        )}

        {(state.view === 'community' || state.view === 'traders') && (
          <div className="bento-card rounded-[2.5rem] p-6 sm:p-8 shadow-sm">
            <div className="flex items-start space-x-4">
              {renderAvatar(state.user ? state.user.avatar : 'U', "w-12 h-12 rounded-full flex-shrink-0 text-base shadow-sm", state.user?.name[0])}
              <div className="flex-1">
                <input type="text" value={quickTitle} onChange={(e) => setQuickTitle(e.target.value)} placeholder={state.view === 'traders' ? 'Drop a trade signal...' : 'Start a discussion...'} className="w-full bg-transparent text-prime font-bold text-lg sm:text-xl outline-none placeholder-[var(--text-muted)] mb-2" />
                <textarea rows="2" value={quickDesc} onChange={(e) => setQuickDesc(e.target.value)} placeholder="What are your thoughts?" className="w-full bg-transparent text-sm sm:text-base text-prime outline-none resize-none placeholder-[var(--text-muted)] font-medium whitespace-pre-wrap"></textarea>
                
                {quickImage && (
                  <div className="relative mt-4 mb-2 w-fit">
                    <img src={quickImage} alt="Preview" className="h-40 rounded-2xl border border-[var(--border-line)] object-cover shadow-sm" />
                    <button onClick={() => setQuickImage(null)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:scale-110 transition relative z-10"><i data-lucide="x" className="w-4 h-4 pointer-events-none"></i></button>
                  </div>
                )}
                
                <div className="flex justify-between items-center pt-4 border-t border-[var(--border-line)] mt-4">
                  <div className="flex space-x-2 text-sub">
                    <input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageSelect} className="hidden" />
                    <button onClick={() => fileInputRef.current?.click()} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition" title="Attach Image"><i data-lucide="image" className="w-5 h-5 pointer-events-none"></i></button>
                  </div>
                  <button onClick={handleQuickPost} className="btn-press px-8 py-3 rounded-xl text-white font-bold text-sm shadow-md relative z-10" style={{ background: 'var(--primary-glow)' }}>Post</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 border border-dashed border-[var(--border-line)] rounded-3xl">
            ...
            <button onClick={() => { setActiveFilter('all'); setSearchQuery(''); setDebouncedSearch(''); }} className="mt-4 px-5 py-2.5 rounded-xl surface-bg border border-[var(--border-line)] text-xs text-[var(--primary-glow)] font-bold hover-lift relative z-10">Clear Filters</button>
          </div>
        ) : (
          <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8" : "flex flex-col space-y-4 sm:space-y-6 lg:space-y-8"}>
            {filteredData.map(item => {
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[240px] lg:h-[280px] group relative overflow-hidden">
                ...
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer relative group">
                  <div className="flex justify-between items-start mb-4">
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 flex items-center space-x-3 hover:opacity-80 transition cursor-pointer w-fit">
                      ...
                      <div className="flex flex-col pointer-events-none"><span className="text-sm sm:text-base font-bold text-prime hover:underline break-words">{item.host}</span><span className="text-[9px] sm:text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5"><i data-lucide="shield-check" className="w-3 h-3 mr-1 pointer-events-none"></i> Verified</span></div>
                    </div>
                    <button onClick={(e) => handleReport(e, item)} className="relative z-10 text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 hover:bg-red-500/10 rounded-xl" title="Report"><i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i></button>
                  </div>
                  
                  <h3 className="text-lg sm:text-xl lg:text-3xl font-bold text-prime mb-3 leading-tight break-words pointer-events-none">{item.title}</h3>
                  <p className="text-xs sm:text-sm lg:text-base text-sub mb-6 leading-relaxed font-medium break-words whitespace-pre-wrap pointer-events-none">{item.desc}</p>
                  
                  {item.image && (
                    <div className="mb-6 rounded-3xl overflow-hidden border border-[var(--border-line)] max-h-96 pointer-events-none">
                  ...
                  <div className="flex items-center space-x-6 text-sub text-xs sm:text-sm font-medium border-t border-[var(--border-line)] pt-5">
                    {state.view === 'community' && (
                      <><button onClick={(e) => handleLike(e, item.id)} className={`relative z-10 btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-white'}`}><i data-lucide="heart" className={`w-5 h-5 mr-2 pointer-events-none transition-all ${likedPosts[item.id] ? 'fill-red-500 animate-heart-burst' : ''}`}></i> {likedPosts[item.id] ? (item.likes || 0) + 1 : (item.likes || 0)}</button><span className="flex items-center hover:text-white transition pointer-events-none"><i data-lucide="message-circle" className="w-5 h-5 mr-2"></i> {item.comments || 0}</span></>
                    )}
                    {state.view === 'traders' && (
                      <span className={`flex items-center px-4 py-1.5 rounded-full surface-bg border border-[var(--border-line)] shadow-sm text-[10px] sm:text-xs font-bold pointer-events-none ${item.sentiment === 'bullish' ? 'text-green-500' : 'text-red-500'}`}><i data-lucide={item.sentiment === 'bullish' ? 'trending-up' : 'trending-down'} className="w-4 h-4 mr-1.5 pointer-events-none"></i> {item.sentiment?.toUpperCase()}</span>
                    )}
                    {state.view === 'news' && (
                      <div className="flex items-center space-x-3"><span className="font-mono text-[9px] sm:text-[10px] surface-bg border border-[var(--border-line)] px-2.5 py-1 rounded-md shadow-sm pointer-events-none">SRC: {item.source || 'SYS'}</span><button className="relative z-10 text-[10px] sm:text-xs font-bold text-[var(--primary-glow)] hover:underline flex items-center">Read Article <i data-lucide="external-link" className="w-3.5 h-3.5 ml-1.5 pointer-events-none"></i></button></div>
                    )}
                    <button onClick={(e) => handleShare(e)} className="relative z-10 btn-press text-sub hover:text-[var(--primary-glow)] p-2 rounded-xl hover:bg-white/5 transition"><i data-lucide="-2" className="w-5 h-5 pointer-events-none"></i></button>
                  </div>
                </div>
              );
            })}
      ...
      <aside className="hidden lg:block w-72 xl:w-80 shrink-0">
        <div className="sticky top-[88px] space-y-6">
          <div className="glass-panel border rounded-3xl p-6 hover-lift">
            <h3 className="text-sm font-bold text-prime mb-5 flex items-center border-b border-[var(--border-line)] pb-3"><i data-lucide="trending-up" className="w-5 h-5 mr-2 text-[var(--primary-glow)]"></i> Trending Now</h3>
            <div className="space-y-5">
              <div className="cursor-pointer group"><p className="text-sm font-bold text-prime group-hover:text-[var(--primary-glow)] transition">#NFP Week</p><p className="text-[10px] text-sub mt-1">1,245 posts</p></div>
              <div className="cursor-pointer group"><p className="text-sm font-bold text-prime group-hover:text-[var(--primary-glow)] transition">Bitcoin Halving</p><p className="text-[10px] text-sub mt-1">858 posts</p></div>
            </div>
          </div>
          <div className="glass-panel border rounded-3xl p-6 hover-lift">
            <h3 className="text-sm font-bold text-prime mb-5 flex items-center border-b border-[var(--border-line)] pb-3"><i data-lucide="users" className="w-5 h-5 mr-2 text-sub"></i> Top Profiles</h3>
            <div className="space-y-5">
              <div className="flex items-center justify-between"><div className="flex items-center space-x-3"><div className="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white text-sm font-bold">E</div><div><p className="text-sm font-bold text-prime hover:underline cursor-pointer">Elena R.</p><p className="text-[10px] text-sub mt-0.5">FX Mentor</p></div></div><button className="text-[10px] font-bold text-[var(--primary-glow)] bg-[var(--primary-glow)]/10 px-4 py-1.5 rounded-full hover:bg-[var(--primary-glow)] hover:text-white transition">Follow</button></div>
              <div className="flex items-center justify-between"><div className="flex items-center space-x-3"><div className="w-10 h-10 rounded-full bg-purple-500 flex items-center justify-center text-white text-sm font-bold">D</div><div><p className="text-sm font-bold text-prime hover:underline cursor-pointer">David K.</p><p className="text-[10px] text-sub mt-0.5">Algo Trader</p></div></div><button className="text-[10px] font-bold text-[var(--primary-glow)] bg-[var(--primary-glow)]/10 px-4 py-1.5 rounded-full hover:bg-[var(--primary-glow)] hover:text-white transition">Follow</button></div>
            </div>
          </div>
          <div className="text-[10px] text-sub px-2 flex flex-wrap gap-x-4 gap-y-2"><a href="#" className="hover:underline hover:text-prime">Terms</a><a href="#" className="hover:underline hover:text-prime">Privacy Space</a><span>© 2026 Vennamis</span></div>
        </div>
      </aside>
