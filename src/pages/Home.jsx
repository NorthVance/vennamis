...
export default function Home() {
  ...
  // EXEC: Safe Action Handlers (Bubbling Prevented)
      ...
      {state.view === 'gigs' && (
        <div className="text-center space-y-4 sm:space-y-6 pt-4 sm:pt-8 transition-all duration-500">
          <div className="inline-flex items-center space-x-2 px-3 sm:px-4 py-1.5 rounded-full border surface-bg text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-sub shadow-sm">
            <i data-lucide="globe-2" className="w-3.5 h-3.5 pointer-events-none" style={{ color: 'var(--primary-glow)' }}></i>
      ...
      <div className="glass-panel flex flex-col-reverse sm:flex-row justify-between items-center gap-4 sticky top-[72px] z-30 py-3 px-4 sm:px-5 rounded-3xl mt-4 shadow-xl">
        <div className="flex p-1.5 bg-[var(--bg-base)]/50 border border-[var(--border-line)] rounded-2xl w-full sm:w-auto overflow-x-auto hide-scrollbar">
          {['gigs', 'community', 'traders', 'news'].map((nav) => (
            ...
            >
              {nav === 'gigs' && <i data-lucide="briefcase" className="w-4 h-4 pointer-events-none"></i>}
              {nav === 'community' && <i data-lucide="users" className="w-4 h-4 pointer-events-none"></i>}
              {nav === 'traders' && <i data-lucide="trending-up" className="w-4 h-4 pointer-events-none"></i>}
              {nav === 'news' && <i data-lucide="newspaper" className="w-4 h-4 pointer-events-none"></i>}
              <span className="hidden sm:inline pointer-events-none">{nav}</span>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 mt-6 gap-4">
          ...
          <div className="flex items-center space-x-2 overflow-x-auto hide-scrollbar w-full sm:w-auto pb-1 sm:pb-0">
            ...
            <div className="flex items-center space-x-2 border-l border-[var(--border-line)] pl-2 sm:pl-3 ml-1">
              <i data-lucide="arrow-down-up" className="w-3.5 h-3.5 text-sub pointer-events-none"></i>
              ...
              <div className="flex-1">
                ...
                {quickImage && (
                  <div className="relative mt-3 mb-2 w-fit">
                    ...
                    <button onClick={() => setQuickImage(null)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:scale-110 transition"><i data-lucide="x" className="w-3 h-3 pointer-events-none"></i></button>
                  </div>
                )}
                ...
                <div className="flex justify-between items-center pt-3 border-t border-[var(--border-line)] mt-3">
                  <div className="flex space-x-2 text-sub">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition" title="Attach Image"><i data-lucide="image" className="w-4 h-4 pointer-events-none"></i></button>
                    <button type="button" onClick={() => setState(prev => ({...prev, toast: {type:'info', message:'Link attachment disabled.'}}))} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition"><i data-lucide="link" className="w-4 h-4 pointer-events-none"></i></button>
                  </div>
                  <button type="button" onClick={handleQuickPost} className="btn-press px-8 py-2.5 rounded-xl text-white font-bold text-sm shadow-md" style={{ background: 'var(--primary-glow)' }}>Post</button>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-5">
                        ...
                        <button type="button" onClick={(e) => handleReport(e, item)} className="text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report this post">
                    ...
                    <div className="flex justify-between items-end mt-auto pt-5 border-t border-[var(--border-line)]/50">
                      <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="hover:opacity-80 transition cursor-pointer flex items-center space-x-3">
                         ...
                         <div>
                           <p className="text-[9px] text-sub uppercase tracking-wider mb-0.5 font-bold break-words">Host Identity</p>
                           <span className="text-sm font-bold text-prime hover:underline break-words">{item.host}</span>
                         </div>
                      </div>
                      <button type="button" onClick={(e) => handleApply(e, item)} className="btn-press px-5 py-2.5 rounded-xl surface-bg border border-[var(--border-line)] flex items-center justify-center hover:border-[var(--primary-glow)] hover:text-[var(--primary-glow)] text-prime shadow-sm transition-all duration-300 font-bold text-xs gap-2 shrink-0">
                  ...
                  <div className="flex justify-between items-start mb-5">
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="flex items-center space-x-4 hover:opacity-80 transition cursor-pointer w-fit">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5"><i data-lucide="shield-check" className="w-3.5 h-3.5 mr-1 pt-0.5"></i> Verified Content</span>
                      </div>
                    </div>
                    <button type="button" onClick={(e) => handleReport(e, item)} className="text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report this post">
                      <i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i>
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 sm:pt-5">
                    {state.view === 'community' && (
                      <div className="flex space-x-6 text-sub text-xs font-bold">
                        <button type="button" onClick={(e) => handleLike(e, item.id)} className={`btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-[var(--primary-glow)]'}`}>
                        ...
                        <span className="flex items-center hover:text-[var(--primary-glow)] transition"><i data-lucide="message-circle" className="w-4 h-4 sm:w-5 sm:h-5 mr-2 pointer-events-none"></i> {item.comments || 0}</span>
                      </div>
                    )}
                    {state.view === 'traders' && (
                    ...
                    {state.view === 'news' && (
                      <div className="flex items-center space-x-3">
                        ...
                        <button type="button" className="text-[10px] sm:text-xs font-bold text-[var(--primary-glow)] hover:underline flex items-center">Read Article <i data-lucide="external-link" className="w-3.5 h-3.5 ml-1.5 pointer-events-none"></i></button>
                      </div>
                    )}
                    <button type="button" onClick={(e) => handleShare(e)} className="btn-press text-sub hover:text-[var(--primary-glow)] p-2 rounded-xl hover:bg-white/5 transition"><i data-lucide="share-2" className="w-4 h-4 sm:w-5 sm:h-5 pointer-events-none"></i></button>
                  </div>
                </div>
              );
            })}
