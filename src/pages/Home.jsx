...
export default function Home() {
  ...
  const handleQuickPost = async () => {
    ...
    await DatabaseService.createPost({ id: 'p' + Date.now(), type: 'post', host: state.user.name, avatar: state.user.avatar, title: quickTitle, desc: quickDesc, tag: state.view === 'traders' ? 'Signal' : 'Discussion', likes: 0, comments: 0 }, state.view);
    setQuickTitle(''); setQuickDesc(''); setState(prev => ({ ...prev, refreshTick: prev.refreshTick + 1, toast: { type: 'success', message: 'Published!' } }));
  };

  const renderAvatar = (avatarData, sizeClasses) => (avatarData && (avatarData.startsWith('http') || avatarData.startsWith('blob:'))) ? <img src={avatarData} alt="Avatar" className={`object-cover ${sizeClasses}`} /> : <div className={`flex items-center justify-center text-white font-bold bg-gray-800 ${sizeClasses}`}>{avatarData ? avatarData[0] : 'U'}</div>;
