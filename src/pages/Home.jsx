...
import { Search, MapPin, Sparkles, ArrowRight, ShieldCheck, Filter, Clock } from 'lucide-react';
import { rotatingWords } from '../assets/store';

// ==========================================
// 🧠 ADVANCED HOOKS: Anti-Layout Shift Typewriter
// ==========================================
...
// ==========================================
// 🏢 MAIN COMPONENT: Home & Gigs Feed
// ==========================================
export default function Home() {
    const { state, setState, t } = useContext(AppContext);
    
    // Local States for Complex Filtering
    ...
    // Simulate Network Latency for Skeleton Loader (UX Enhancement)
    useEffect(() => {
        setIsLoading(true);
    ...
    // 🧠 COMPLEX MEMOIZATION: High-Performance Filtering Engine
    const filteredData = useMemo(() => {
        const rawData = state.data[state.currentView] || [];
        return rawData.filter(item => {
            // Safe translation fallback check
    ...
    }, [state.data, state.currentView, searchQuery, categoryFilter]);
    ...
    return (
        <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-12 sm:space-y-16">
            ...
            {/* ========================================== */}
            {/* 🌟 HERO SECTION (Strictly for Gigs View) */}
            {/* ========================================== */}
            {state.currentView === 'gigs' && (
                <section className="text-center space-y-6 transition-all duration-500 animate-[fadeIn_0.5s_ease-out]">
                    ...
                    {/* Bulletproof Typewriter (Zero Layout Shift) */}
            ...
            {/* ========================================== */}
            {/* 📋 DYNAMIC FEED SECTION (Gigs Focus) */}
            {/* ========================================== */}
                            ...
                            <div key={n} className="surface-bg border border-[var(--border-line)] rounded-3xl p-6 h-[240px] animate-pulse flex flex-col justify-between">
                                <div>
                                    <div className="flex justify-between mb-4">
                                        <div className="w-20 h-6 bg-slate-700/30 rounded-full"></div>
                                        <div className="w-16 h-6 bg-slate-700/30 rounded-lg"></div>
                                    </div>
                                    <div className="w-3/4 h-6 bg-slate-700/30 rounded-lg mb-3"></div>
                                    <div className="w-full h-4 bg-slate-700/30 rounded-lg mb-2"></div>
                                    <div className="w-5/6 h-4 bg-slate-700/30 rounded-lg"></div>
                                </div>
                                <div className="flex justify-between items-center pt-4 border-t border-[var(--border-line)]">
                                    <div className="w-24 h-5 bg-slate-700/30 rounded-lg"></div>
                                    <div className="w-20 h-5 bg-slate-700/30 rounded-lg"></div>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className={state.currentView === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "flex flex-col space-y-6"}>
                        {filteredData.map(item => (
                            <div key={item.id} 
                                 ...
                                 className="surface-bg border glow-border rounded-3xl p-6 hover-lift cursor-pointer flex flex-col justify-between min-h-[240px] relative group shadow-lg">
                                 
                                {/* Escrow Secured Badge (Anti-Scam Visual) */}
