...
export default function Home() {
  ...
  // 📍 FIX: Strict Event Bounds (Prevent Bubbling)
  ...
  return (
    <div className="space-y-10 sm:space-y-12 max-w-5xl mx-auto pb-20">
      ...
      {state.view === 'gigs' && (
        <div className="text-center space-y-4 sm:space-y-6 pt-4 sm:pt-8 transition-all duration-500"><div className="inline-flex items-center space-x-2 px-3 sm:px-4 py-1.5 rounded-full border surface-bg text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-sub shadow-sm"><i data-lucide="globe-2" className="w-3.5 h-3.5" style={{ color: 'var(--primary-glow)' }}></i><span>{t.badge_secure}</span></div><h1 className="text-5xl sm:text-6xl md:text-7xl font-black tracking-tighter leading-tight text-prime"><span>{t.hero_static}</span><br/><div className="h-[1.2em] mt-1 sm:mt-2 flex justify-center items-center"><Typewriter /></div></h1><p className="text-xs sm:text-base text-sub max-w-2xl mx-auto px-4 font-medium">{t.hero_sub}</p></div>
      )}
                ...
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      ...
                      <h3 className="text-xl sm:text-2xl font-black text-prime mb-2 line-clamp-1 break-words">{item.title}</h3><p className="text-sm text-sub line-clamp-2 leading-relaxed font-medium break-words whitespace-pre-wrap">{item.desc}</p>
                    </div>
                    <div className="flex justify-between items-end mt-auto pt-5 border-t border-[var(--border-line)]/50">
                      <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 hover:opacity-80 transition cursor-pointer flex items-center space-x-3">{renderAvatar(item.avatar || item.host[0], "w-8 h-8 rounded-full text-xs shadow-sm", item.host[0])}<div><p className="text-[9px] text-sub uppercase tracking-wider mb-0.5 font-bold break-words">Host Identity</p><span className="text-sm font-bold text-prime hover:underline break-words">{item.host}</span></div></div>
