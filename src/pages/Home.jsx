...
export default function Home() {
  ...
  // 📍 SYS: Event Blocker (กันการกดทะลุบนหน้าจอมือถือ 100%)
  const handleSafeAction = (e, callback) => {
    e.preventDefault();
    e.stopPropagation();
    callback();
  };
  ...
  const openProfile = (hostName, avatarData) => { 
            ...
            {filteredData.map(item => {
                  ...
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-5">
                        ...
                        {/* 📍 FIX: Safe Action on Report Button */}
                        <button type="button" onClick={(e) => handleSafeAction(e, () => handleReport(item))} className="text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0 z-10" title="Report this post">
                    ...
                    <div className="flex justify-between items-end mt-auto pt-5 border-t border-[var(--border-line)]/50 relative z-10">
                      <div onClick={(e) => handleSafeAction(e, () => openProfile(item.host, item.avatar))} className="hover:opacity-80 transition cursor-pointer flex items-center space-x-3">
                      ...
                      <button type="button" onClick={(e) => handleSafeAction(e, () => handleApply(item))} className="btn-press px-5 py-2.5 rounded-xl surface-bg border border-[var(--border-line)] flex items-center justify-center hover:border-[var(--primary-glow)] hover:text-[var(--primary-glow)] text-prime shadow-sm transition-all duration-300 font-bold text-xs gap-2 shrink-0">
                        Apply <i data-lucide="arrow-up-right" className="w-4 h-4"></i>
                      </button>
                    </div>
                  </div>
                );
              ...
              return (
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer relative group">
                  <div className="flex justify-between items-start mb-5">
                    <div onClick={(e) => handleSafeAction(e, () => openProfile(item.host, item.avatar))} className="flex items-center space-x-4 hover:opacity-80 transition cursor-pointer w-fit z-10 relative">
                    ...
                    {/* 📍 FIX: Safe Action on Report Button */}
                    <button type="button" onClick={(e) => handleSafeAction(e, () => handleReport(item))} className="text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0 z-10 relative" title="Report this post">
                      <i data-lucide="flag" className="w-4 h-4"></i>
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 sm:pt-5 relative z-10">
                    {state.view === 'community' && (
                      <div className="flex space-x-6 text-sub text-xs font-bold">
                        <button type="button" onClick={(e) => handleSafeAction(e, () => handleLike(item.id))} className={`btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-[var(--primary-glow)]'}`}>
                    ...
                    {state.view === 'news' && (
                      <div className="flex items-center space-x-3">
                        ...
                        <button onClick={(e) => handleSafeAction(e, () => {})} className="text-[10px] sm:text-xs font-bold text-[var(--primary-glow)] hover:underline flex items-center">Read Article <i data-lucide="external-link" className="w-3.5 h-3.5 ml-1.5"></i></button>
                      </div>
                    )}
                    
                    <button type="button" onClick={(e) => handleSafeAction(e, handleShare)} className="btn-press text-sub hover:text-[var(--primary-glow)] p-2 rounded-xl hover:bg-white/5 transition">
