...
export default function Home() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 max-w-[1400px] mx-auto pb-20">
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between items-center pt-3 border-t border-[var(--border-line)] mt-3">
                  <div className="flex space-x-2 text-sub">
                    ...
                    <button onClick={() => setState(prev => ({...prev, toast: {type:'info', message:'Link attachment disabled.'}}))} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition"><i data-lucide="link" className="w-4 h-4 pointer-events-none"></i></button>
                  </div>
            ...
            {filteredData.map(item => {
              ...
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 cursor-pointer flex flex-col justify-between h-[240px] group relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--primary-glow)] opacity-0 group-hover:opacity-10 blur-[60px] transition-opacity duration-500 rounded-full pointer-events-none"></div>
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] font-bold uppercase text-sub bg-white/5 px-3 py-1 rounded-full border border-[var(--border-line)] pointer-events-none">{item.loc}</span>
                        <button onClick={(e) => handleReport(e, item)} className="text-sub hover:text-red-500 opacity-0 group-hover:opacity-100 transition relative z-10"><i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i></button>
                      </div>
                      <h3 className="text-xl font-bold text-prime mb-2 line-clamp-1 pointer-events-none">{item.title}</h3>
                      <p className="text-xs text-sub line-clamp-2 leading-relaxed pointer-events-none">{item.desc}</p>
                    </div>
                    <div className="flex justify-between items-end mt-auto pt-4 border-t border-[var(--border-line)]/50">
                      <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="hover:opacity-80 transition cursor-pointer relative z-10">
                        <p className="text-[10px] text-sub uppercase tracking-wider mb-1 pointer-events-none">Host Identity</p>
                        <span className="text-sm font-bold text-prime flex items-center hover:underline pointer-events-none">
                          {renderAvatar(item.avatar || item.host[0], "w-5 h-5 rounded-full mr-2 text-[8px]")}
                          {item.host}
                        </span>
                      </div>
                      <button onClick={(e) => handleApply(e, item)} className="btn-press w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-[var(--primary-glow)] text-white transition-colors duration-300 relative z-10">
                        <i data-lucide="arrow-up-right" className="w-5 h-5 pointer-events-none"></i>
                      </button>
                    </div>
                  </div>
                );
              ...
              return (
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 cursor-pointer relative group">
                  <button onClick={(e) => handleReport(e, item)} className="absolute top-6 right-6 text-sub hover:text-red-500 opacity-0 group-hover:opacity-100 transition relative z-10"><i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i></button>
                  
                  <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="flex items-center space-x-3 mb-4 hover:opacity-80 transition cursor-pointer w-fit relative z-10">
                    {renderAvatar(item.avatar || item.host[0], "w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center font-bold text-white text-sm")}
                  ...
                  <h3 className="text-lg sm:text-xl font-bold text-prime mb-2 pointer-events-none">{item.title}</h3>
                  <p className="text-sm text-sub mb-6 leading-relaxed pointer-events-none whitespace-pre-wrap break-words">{item.desc}</p>
                  ...
                  <div className="flex items-center space-x-6 text-sub text-xs font-medium border-t border-[var(--border-line)] pt-4">
                    {state.view === 'community' && (
                      <>
                        <button onClick={(e) => handleLike(e, item.id)} className={`btn-press flex items-center transition relative z-10 ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-white'}`}>
                          <i data-lucide="heart" className={`w-4 h-4 mr-2 pointer-events-none ${likedPosts[item.id] ? 'fill-red-500 animate-heart-burst' : ''}`}></i> {likedPosts[item.id] ? (item.likes || 0) + 1 : (item.likes || 0)}
                        </button>
                    ...
                    {state.view === 'traders' && (
                      <span className={`flex items-center px-3 py-1 rounded-full bg-white/5 border border-[var(--border-line)] pointer-events-none ${item.sentiment === 'bullish' ? 'text-green-500' : 'text-red-500'}`}>
