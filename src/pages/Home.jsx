...
export default function Home() {
  ...
  // 📍 FIX: Safe Action Handlers (ป้องกันบั๊กกดทะลุชั้น - Event Bubbling)
  ...
  const handleApply = (e, item) => { 
    e.preventDefault(); 
    e.stopPropagation(); // หยุดการคลิกไม่ให้หลุดไปโดน Card
  ...
  // 📍 FIX: Report Action (แก้ปัญหาเด้งไปหน้า Post ก่อน)
  const handleReport = (e, item) => { 
    e.preventDefault(); 
    e.stopPropagation(); // หยุดการคลิกไม่ให้ทะลุ 100%
    if (!state.user) return setState(prev => ({ ...prev, activeModal: 'modal-login' }));
        ...
        {isLoading ? (
          ...
          <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "flex flex-col space-y-6"}>
            {filteredData.map(item => {
              ...
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => handleCardClick(item)} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer flex flex-col justify-between h-[260px] group relative overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-5">
                        ...
                        {/* 📍 FIX: Report Button (Safely Handled) */}
              ...
              return (
                <div key={item.id} onClick={() => handleCardClick(item)} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer relative group">
                  <div className="flex justify-between items-start mb-5">
