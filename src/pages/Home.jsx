...
export default function Home() {
  ...
  return (
    // 📍 UI: ปรับระยะห่างเป็น md:space-y-12 และ space-y-6 บนมือถือ
    <div className="space-y-6 md:space-y-12 max-w-5xl mx-auto pb-20">
      
      {state.view === 'gigs' && (
        <div className="text-center space-y-4 md:space-y-6 pt-2 md:pt-8 transition-all duration-500">
          ...
          <h1 className="text-4xl md:text-7xl font-black tracking-tighter leading-tight text-prime">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="glass-panel border p-1.5 rounded-xl md:rounded-2xl flex flex-col sm:flex-row gap-2 shadow-xl hover-lift">
          <div className="flex-1 flex items-center px-4 py-2">
            <i data-lucide="search" className="w-4 h-4 sm:w-5 sm:h-5 text-sub mr-3"></i>
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent border-none outline-none text-prime placeholder-[var(--text-muted)] text-sm sm:text-base font-medium" placeholder="Search skills, posts, or news..." />
          </div>
        </div>
      </div>
      ...
      <section>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 mt-6 gap-4">
          <div className="flex items-center space-x-2 sm:space-x-3"><span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full animate-pulse" style={{ backgroundColor: 'var(--primary-glow)', boxShadow: '0 0 10px var(--primary-glow)' }}></span><h2 className="text-xl sm:text-2xl font-black text-prime uppercase tracking-wider">{state.view} Stream</h2></div>
          <div className="flex items-center space-x-2 overflow-x-auto hide-scrollbar w-full sm:w-auto pb-1 sm:pb-0"><button onClick={() => setActiveFilter('all')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap ${activeFilter === 'all' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>All</button>{state.view === 'gigs' && (<><button onClick={() => setActiveFilter('remote')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap ${activeFilter === 'remote' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}>Remote Only</button><button onClick={() => setActiveFilter('high_budget')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'high_budget' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}><i data-lucide="flame" className="w-3 h-3 mr-1.5 pointer-events-none"></i> High Budget</button></>)}{state.view === 'community' && <button onClick={() => setActiveFilter('top_rated')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'top_rated' ? 'bg-[var(--primary-glow)] text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}><i data-lucide="trending-up" className="w-3 h-3 mr-1.5 pointer-events-none"></i> Top Rated</button>}{state.view === 'traders' && <button onClick={() => setActiveFilter('bullish')} className={`btn-press px-4 py-2 rounded-lg text-[10px] sm:text-xs font-bold transition whitespace-nowrap flex items-center ${activeFilter === 'bullish' ? 'bg-green-500 text-white shadow-md' : 'surface-bg border border-[var(--border-line)] text-sub hover:text-prime hover:bg-white/5'}`}><i data-lucide={state.view === 'traders' ? "trending-up" : "briefcase"} className="w-3 h-3 mr-1.5 pointer-events-none"></i> Bullish Intel</button>}<div className="flex items-center space-x-2 border-l border-[var(--border-line)] pl-2 sm:pl-3 ml-1"><i data-lucide="arrow-down-up" className="w-3.5 h-3.5 text-sub"></i><select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent text-[10px] sm:text-xs font-bold text-prime outline-none cursor-pointer"><option value="newest" className="bg-[var(--bg-surface)]">Newest</option>{state.view === 'gigs' && <option value="price_desc" className="bg-[var(--bg-surface)]">Highest</option>}{(state.view === 'community' || state.view === 'gigs') && <option value="popular" className="bg-[var(--bg-surface)]">Popular</option>}</select></div>{state.view === 'news' && <button onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-add-news' }))} className="btn-press px-4 py-2 rounded-xl text-white text-[10px] sm:text-xs font-bold shadow-md flex items-center gap-1.5 hover-lift whitespace-nowrap ml-auto" style={{ background: 'var(--primary-glow)' }}><i data-lucide="plus" className="w-3.5 h-3.5 pointer-events-none"></i> Add News</button>}</div>
        </div>

        {/* QUICK POST */}
        {(state.view === 'community' || state.view === 'traders') && (
          <div className="bento-card rounded-[2rem] p-4 sm:p-5 mb-6 sm:mb-8">
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <textarea rows="2" value={quickDesc} onChange={(e) => setQuickDesc(e.target.value)} placeholder="What are your thoughts?" className="w-full bg-transparent text-sm text-prime outline-none resize-none placeholder-[var(--text-muted)] font-medium whitespace-pre-wrap"></textarea>{quickImage && (<div className="relative mt-3 mb-2 w-fit"><img src={quickImage} alt="Preview" className="h-32 rounded-xl border border-[var(--border-line)] object-cover shadow-sm" /><button onClick={() => setQuickImage(null)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:scale-110 transition relative z-10"><i data-lucide="x" className="w-3 h-3 pointer-events-none"></i></button></div>)}<div className="flex justify-between items-center pt-3 border-t border-[var(--border-line)] mt-3"><div className="flex space-x-2 text-sub"><input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageSelect} className="hidden" /><button onClick={() => fileInputRef.current?.click()} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition" title="Attach Image"><i data-lucide="image" className="w-4 h-4 pointer-events-none"></i></button><button onClick={() => setState(prev => ({...prev, toast: {type:'info', message:'Link disabled.'}}))} className="btn-press p-2 hover:text-prime hover:bg-[var(--border-line)] rounded-xl transition"><i data-lucide="link" className="w-4 h-4 pointer-events-none"></i></button></div><button onClick={handleQuickPost} className="btn-press px-8 py-2.5 rounded-xl text-white font-bold text-sm shadow-md relative z-10" style={{ background: 'var(--primary-glow)' }}>Post</button></div></div></div></div>
        )}

        {/* FEED */}
        {isLoading ? <Skeleton view={state.view} /> : filteredData.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-[var(--border-line)] rounded-3xl"><i data-lucide="filterX" className="w-8 h-8 text-sub mx-auto mb-3 opacity-50"></i><p className="text-sm font-medium text-sub">No results match filter.</p><button onClick={() => { setActiveFilter('all'); setSearchQuery(''); }} className="mt-4 px-4 py-2 rounded-lg surface-bg border border-[var(--border-line)] text-xs text-[var(--primary-glow)] font-bold hover-lift relative z-10">Clear Filters</button></div>
        ) : (
          <div className={state.view === 'gigs' ? "grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6" : "flex flex-col space-y-4 sm:space-y-6"}>
            {filteredData.map(item => {
              if (state.view === 'gigs') {
                return (
                  <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-5 sm:p-6 cursor-pointer flex flex-col justify-between h-[200px] sm:h-[220px] relative overflow-hidden">
                    ...
                    <div><div className="flex justify-between items-start mb-4"><span className="text-[10px] font-bold uppercase text-sub surface-bg px-3 py-1.5 rounded-full border border-[var(--border-line)] shadow-sm">{item.loc}</span><button onClick={(e) => handleReport(e, item)} className="relative z-10 text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report"><i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i></button></div><h3 className="text-lg sm:text-2xl font-black text-prime mb-1.5 line-clamp-1 break-words">{item.title}</h3><p className="text-xs sm:text-sm text-sub line-clamp-2 leading-relaxed font-medium break-words whitespace-pre-wrap">{item.desc}</p></div>
                    <div className="flex justify-between items-end mt-auto pt-3 sm:pt-4 border-t border-[var(--border-line)]/50"><div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 hover:opacity-80 transition cursor-pointer flex items-center space-x-3">{renderAvatar(item.avatar || item.host[0], "w-8 h-8 rounded-full text-xs shadow-sm", item.host[0])}<div><p className="text-[9px] text-sub uppercase tracking-wider mb-0.5 font-bold break-words">Host Identity</p><span className="text-sm font-bold text-prime hover:underline break-words">{item.host}</span></div></div><button onClick={(e) => handleApply(e, item)} className="relative z-10 btn-press px-5 py-2.5 rounded-xl surface-bg border border-[var(--border-line)] flex items-center justify-center hover:border-[var(--primary-glow)] hover:text-[var(--primary-glow)] text-prime shadow-sm transition-all duration-300 font-bold text-xs gap-2 shrink-0">Apply <i data-lucide="arrow-up-right" className="w-4 h-4 pointer-events-none"></i></button></div>
                  </div>
                );
              }
              return (
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-5 sm:p-6 hover-lift glow-border cursor-pointer relative">
                  <div className="flex justify-between items-start mb-4">
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="relative z-10 flex items-center space-x-4 hover:opacity-80 transition cursor-pointer w-fit">{renderAvatar(item.avatar || item.host[0], "w-12 h-12 rounded-full text-base shadow-sm", item.host[0])}<div className="flex flex-col"><span className="text-base font-bold text-prime hover:underline break-words">{item.host}</span><span className="text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5"><i data-lucide="shield-check" className="w-3.5 h-3.5 mr-1 pointer-events-none"></i> Verified</span></div></div>
                  ...
                  <h3 className="text-xl sm:text-2xl font-black text-prime mb-3 leading-tight break-words">{item.title}</h3><p className="text-sm sm:text-base text-sub mb-6 leading-relaxed font-medium break-words whitespace-pre-wrap">{item.desc}</p>
                  {item.image && (<div className="mb-6 rounded-2xl overflow-hidden border border-[var(--border-line)] max-h-96"><img src={item.image} alt="Attachment" className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.style.display='none'; }} /></div>)}
                  <div className="flex justify-between items-center border-t border-[var(--border-line)] pt-4 sm:pt-5">
                    {state.view === 'community' && (<div className="flex space-x-6 text-sub text-xs font-bold"><button onClick={(e) => handleLike(e, item.id)} className={`relative z-10 btn-press flex items-center transition ${likedPosts[item.id] ? 'text-red-500' : 'hover:text-[var(--primary-glow)]'}`}><i data-lucide="heart" className={`w-4 h-4 sm:w-5 sm:h-5 mr-2 pointer-events-none transition-all ${likedPosts[item.id] ? 'fill-red-500 animate-heart-burst' : ''}`}></i> {likedPosts[item.id] ? (item.likes || 0) + 1 : (item.likes || 0)}</button><span className="flex items-center hover:text-[var(--primary-glow)] transition"><i data-lucide="message-circle" className="w-4 h-4 sm:w-5 sm:h-5 mr-2"></i> {item.comments || 0}</span></div>)}
                    {state.view === 'traders' && <span className={`flex items-center px-4 py-1.5 rounded-full surface-bg border border-[var(--border-line)] shadow-sm text-xs ${item.sentiment === 'bullish' ? 'text-green-500' : 'text-red-500'}`}><i data-lucide={item.sentiment === 'bullish' ? 'trending-up' : 'trending-down'} className="w-4 h-4 mr-2 pointer-events-none"></i> {item.sentiment?.toUpperCase()}</span>}
                    {state.view === 'news' && <div className="flex items-center space-x-3"><span className="font-mono text-[10px] sm:text-xs surface-bg border border-[var(--border-line)] px-2.5 py-1 rounded-md shadow-sm">SRC: {item.source || 'SYS'}</span><button className="relative z-10 text-[10px] sm:text-xs font-bold text-[var(--primary-glow)] hover:underline flex items-center">Read Article <i data-lucide="external-link" className="w-3.5 h-3.5 ml-1.5 pointer-events-none"></i></button></div>}
                    <button onClick={(e) => handleShare(e)} className="relative z-10 btn-press text-sub hover:text-[var(--primary-glow)] p-2 rounded-xl hover:bg-white/5 transition"><i data-lucide="share-2" className="w-4 h-4 sm:w-5 sm:h-5 pointer-events-none"></i></button>
                  </div>
                </div>
              );
            })}
