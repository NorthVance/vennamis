    ...
    <div className="space-y-10 sm:space-y-12 max-w-5xl mx-auto pb-20">
      ...
      <section>
        ...
        {isLoading ? <Skeleton view={state.view} /> : filteredData.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-[var(--border-line)] rounded-3xl"><i data-lucide="filterX" className="w-8 h-8 text-sub mx-auto mb-3 opacity-50"></i><p className="text-sm font-medium text-sub">No results match filter.</p><button onClick={() => { setActiveFilter('all'); setSearchQuery(''); }} className="mt-4 px-4 py-2 rounded-lg surface-bg border border-[var(--border-line)] text-xs text-[var(--primary-glow)] font-bold hover-lift">Clear Filters</button></div>
        ) : (
              ...
              return (
                <div key={item.id} onClick={() => setState(prev => ({ ...prev, activeModal: 'modal-gig-detail', selectedItem: item }))} className="btn-press bento-card rounded-[2rem] p-6 sm:p-8 cursor-pointer relative group">
                  <div className="flex justify-between items-start mb-5">
                    <div onClick={(e) => openProfile(e, item.host, item.avatar)} className="flex items-center space-x-4 hover:opacity-80 transition cursor-pointer w-fit">{renderAvatar(item.avatar || item.host[0], "w-12 h-12 rounded-full text-base shadow-sm", item.host[0])}<div className="flex flex-col"><span className="text-base font-bold text-prime hover:underline break-words">{item.host}</span><span className="text-[10px] text-[var(--primary-glow)] flex items-center font-bold mt-0.5"><i data-lucide="shield-check" className="w-3.5 h-3.5 mr-1"></i> Verified Content</span></div></div>
                    <button onClick={(e) => handleReport(e, item)} className="text-gray-400 opacity-60 hover:opacity-100 hover:text-red-500 transition p-2 bg-black/5 hover:bg-red-500/10 rounded-xl border border-transparent hover:border-red-500/30 shrink-0" title="Report this post"><i data-lucide="flag" className="w-4 h-4 pointer-events-none"></i></button>
                  </div>
