...
export default function Workspace() {
  ...
  return (
    <div className="space-y-8 sm:space-y-10 max-w-5xl mx-auto pb-20 animate-modal-pop">
      ...
      {/* UI: Escrow Stats Bento */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        <div className="bento-card rounded-3xl p-6 flex flex-col justify-between h-40">
        ...
        <div className="bento-card rounded-3xl p-6 flex flex-col justify-between h-40 border-[var(--primary-glow)]/30 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--primary-glow)] opacity-10 blur-[50px] rounded-full"></div>
      ...
      <div>
        ...
        <div className="flex flex-col space-y-4">
          {activeContracts.map(contract => (
            <div key={contract.id} className="btn-press bento-card rounded-[2rem] p-5 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group cursor-pointer">
              ...
              <div className="flex items-center justify-between w-full sm:w-auto sm:space-x-8">
                ...
                <button onClick={(e) => { e.stopPropagation(); alert('Delivery Modal goes here!'); }} className="btn-press px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-md transition" style={{ background: 'var(--primary-glow)' }}>
                  Deliver
                </button>
              </div>
            </div>
