...
// 📍 CFG: Socket Connection (เชื่อมกับ URL ของ Render มึง)
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    socket.on('receive_message', (data) => {
      ...
      if (!state.isChatOpen || state.chatHost !== data.senderName) {
        setState(prev => ({ ...prev, toast: { type: 'info', message: `New message from ${data.senderName}` } }));
      }
    });
  ...
  const openRoom = (hostName) => {
    ...
    socket.emit('join_room', hostName); // Join specific room
  ...
  // EXEC: Send Real-Time Chat
  const sendChat = () => {
    if (!chatInput.trim() || !state.chatHost) return;
    const msgData = {
      id: `m-${Date.now()}`, room: state.chatHost, senderName: state.user ? state.user.name : 'Guest', sender: 'me', text: chatInput, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})
    };
    
    socket.emit('send_message', msgData); // Push to Server
  ...
  return (
    <>
      <button onClick={toggleChat} className="btn-press fixed bottom-6 right-6 p-4 rounded-full text-white shadow-[0_0_20px_var(--grid-color)] hover:scale-110 transition-transform z-40" style={{ background: 'var(--primary-glow)' }}>
        <i data-lucide="message-square" className="w-6 h-6"></i>
        {contacts.reduce((acc, c) => acc + c.unread, 0) > 0 && <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-red-500 border-2 border-[var(--bg-base)] rounded-full"></span>}
      </button>
      ...
      <div className={`fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-2rem)] sm:w-[360px] h-[500px] glass-panel border rounded-3xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 z-50 ${state.isChatOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-5 pointer-events-none'}`}>
        <div className="p-4 border-b border-[var(--border-line)] flex justify-between items-center bg-black/5 backdrop-blur-md">
          ...
          <button onClick={closeChat} className="btn-press text-sub hover:text-prime transition bg-white/5 p-1.5 rounded-full"><i data-lucide="x" className="w-4 h-4"></i></button>
        </div>
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-black/10">
            ...
            <div className="p-3 bg-[var(--bg-surface)] border-t border-[var(--border-line)]">
              <div className="flex items-center gap-2 bg-black/20 border border-[var(--border-line)] rounded-2xl p-1 pr-1.5 focus-within:border-[var(--primary-glow)] transition-colors">
                ...
                <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendChat()} placeholder="Secure message..." className="flex-1 bg-transparent text-xs text-prime outline-none placeholder-[var(--text-muted)]" />
